...
const sumEvens = (numbers) => {
  let converted = stringsToNumbs(numbers)
  let filtered = stringsToNumbs(numbers).filter(num => num % 2 === 0)
...
let converted = stringsToNumbs(numbers)
  ...
  for(let i = 0; i < converted.length; i ++) {
    if(converted[i] + i === 512){
      console.log(`index ${i}`)
      console.log(`item ${converted[i]}`)
      return
    }
...
const findWeatherStates = (arr) => {
let weatherStates = []
for (let i = 0; i < arr.length; i++){
  if(weatherStates.indexOf(arr[i].weather_state_name) === -1) {
    weatherStates.push(arr[i].weather_state_name)
  }
